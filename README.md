...
4. 新粘贴的模板图会按 `配置位-当前数量-日期时间.png` 命名，例如 `1-11-20260701194530123456789.png`。
...
8. 在 `模板最多张` 填 10、20、50、100 等上限；新粘贴图片前会先删旧留新，再按删除后的实际数量命名。
...
最右侧 `来源预览` 会显示已选屏幕和应用窗口；应用窗口预览优先使用 Windows DWM 缩略图，由系统合成实时画面，不再靠 120ms 截图轮询。已选应用即使未启动也会保留；下次应用启动后会按窗口名和同名序号自动关联，匹配时找不到窗口就跳过。同名窗口按标题和窗口句柄排序后编号，例如 `浏览器 #2`，避免随机选中。
