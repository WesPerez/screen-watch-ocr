...
7. 也可以在 `监控应用` 里选择一个或多个应用窗口；右侧 `已选来源` 会显示所选屏幕或应用的动态缩略图。
...
`监控应用` 使用 Windows 窗口截图接口，适合被其他窗口遮挡的普通桌面应用；少数硬件加速窗口或最小化后不再绘制画面的应用，系统可能只能返回黑屏或旧画面。
