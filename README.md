...
3. 上方区域会以固定缩略图网格展示模板，默认约两排、每排五张；每张图可单独勾选是否参与匹配，点缩略图后可删除。
4. 模板图会按 `配置位-顺序号-时间戳.png` 命名，例如 `1-10-20260701194530123456.png`；删除中间图片后会按当前显示顺序自动补齐编号，再粘贴的新图接在最后。
5. 选择 `配置位` 1-5；关闭时会自动保存当前配置位的图片、屏幕、应用窗口、区域和匹配参数，下次自动恢复上次配置位。
6. 选择要监控的屏幕；区域留空宽高就是整屏，填宽高就是只扫指定区域。
7. 也可以在 `监控应用` 下拉框里选择一个或多个应用窗口；已选择的应用会留在下方，点 `×` 或在下拉框里再次选择即可取消。
8. 在 `模板最多张` 填 10、20、50、100 等上限；新粘贴图片前会先删旧留新，再把保留图片重排为连续编号。
9. 在 `蜂鸣秒` 填 3、5 等持续时间；蜂鸣没结束时再次命中不会重新计时。
10. 在 `截图最多张` 填 10、20、50、100 等上限；产生新截图时会自动只保留最新的 N 张。
11. 点 `开始监控`，按钮会切换为 `停止监控`；命中后蜂鸣、写日志、保存带红框截图。

最右侧 `来源预览` 会显示已选屏幕和应用窗口；屏幕来源会显示真实截图预览，应用窗口预览优先使用 Windows DWM 原生实时缩略图，DWM 不可用时才退回截图预览。已选应用即使未启动也会保留；下次应用启动后会按窗口名和同名序号自动关联，匹配时找不到窗口就跳过。同名窗口按标题和窗口句柄排序后编号，例如 `浏览器 #2`，避免随机选中。

点击窗口右上角关闭按钮会缩小到系统托盘；托盘双击或右键 `打开` 可恢复窗口，右键 `退出` 才会真正结束程序。监控中托盘图标中间区域会变为绿色。
...
- `缩放`: 模板按哪些倍率去找，例如 `1.0`、`0.9,1.0,1.1`、`0.5-2.0:0.1` 或 `0.1-2.0:5%`；倍率最多展开 120 个，避免全屏扫描过慢。
- `间隔ms`: 每轮截屏扫描之间的间隔，越小越实时。
- `同图冷却秒`: 同一屏幕、同一张模板命中后，多少秒内不重复报警和存图。
- `蜂鸣秒`: 一次有效报警后蜂鸣持续多久；蜂鸣未结束时再次命中不会重置时长。
- `模板最多张`: 当前配置位最多保留多少张待匹配图片；新增图片时自动删最旧的模板。
- `截图最多张`: 截图目录最多保留多少张；下次保存截图时自动删旧留新。

`监控应用` 使用 Windows 窗口截图接口，适合被其他窗口遮挡的普通桌面应用；最小化窗口会尝试用系统接口抓取，但少数硬件加速窗口最小化后不再绘制画面，系统可能只能返回黑屏或旧画面。
