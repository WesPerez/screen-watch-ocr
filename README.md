...
最右侧 `来源预览` 会显示已选屏幕和应用窗口；应用窗口预览优先使用 Windows DWM 原生实时缩略图，DWM 不可用时才退回截图预览。已选应用即使未启动也会保留；下次应用启动后会按窗口名和同名序号自动关联，匹配时找不到窗口就跳过。同名窗口按标题和窗口句柄排序后编号，例如 `浏览器 #2`，避免随机选中。
